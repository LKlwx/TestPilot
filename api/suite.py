...
from models import TestSuite, TestCase, TestTask, User, suite_case_association
...
def get_suite(sid):
    ...
    rows = db.session.query(suite_case_association).filter_by(suite_id=sid).order_by(suite_case_association.c.suite_id).all()
    cases = []
    for row in rows:
        case = None
...
def _sync_cases(suite, cases):
    ...
    for item in cases:
        db.session.execute(suite_case_association.insert().values(
            suite_id=suite.id,
