...
@coverage_bp.route("/coverage/api/page")
def coverage_page():
...
def _mark_covered(method: str, url: str, user_id: int):
    ...
    # 2. 参数化路径匹配：将 Swagger 的 {id} / :id 替换为 [^/]+ 正则
...
@coverage_bp.route("/coverage/api/stats", methods=["GET"])
@jwt_required()
def coverage_stats():
    """覆盖率统计"""
...
@coverage_bp.route("/coverage/api/list", methods=["GET"])
@jwt_required()
def coverage_list():
...
@coverage_bp.route("/coverage/api/import", methods=["POST"])
@jwt_required()
def import_swagger():
    ...
    for path, methods in paths.items():
        for method, detail in methods.items():
            ...
            exists = ApiCoverage.query.filter_by(method=method.upper(), path=path).first()
            if not exists:
                api = ApiCoverage(
...
@coverage_bp.route("/coverage/api/<int:aid>", methods=["DELETE"])
@jwt_required()
