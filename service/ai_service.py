...
class AIService:
    ...
    def generate_api(scene, user_id):
        ...
        try:
            ...
            raise
        task = AIAgentTask(task_type="generate_api", input_content=scene, output_result=str(res), creator_id=user_id)
        db.session.add(task)
        try:
            db.session.commit()
    ...
    def generate_ui(scene, user_id):
        ...
        try:
            ...
            raise
        task = AIAgentTask(task_type="generate_ui", input_content=scene, output_result=str(res), creator_id=user_id)
        db.session.add(task)
        try:
            db.session.commit()
        except SQLAlchemyError:
    ...
    def analyze_log(log, user_id):
        ...
        try:
            ...
            raise
        task = AIAgentTask(task_type="analyze_failure", input_content=log, output_result=res, creator_id=user_id)
        db.session.add(task)
        try:
            db.session.commit()
        except SQLAlchemyError:
    ...
    def save_ui(data, user_id):
        ...
        ui = UICase(name=data["name"], url=data["url"], steps=data.get("steps", ""), creator_id=user_id)
        db.session.add(ui)
        try:
            db.session.commit()
        except SQLAlchemyError:
    ...
    def get_history(task_type, user_id, limit=20):
        """查询当前用户的 AI 任务历史记录"""
        tasks = (
            AIAgentTask.query.filter_by(task_type=task_type, creator_id=user_id)
            .order_by(AIAgentTask.create_time.desc())
            .limit(limit)
            .all()
        )
